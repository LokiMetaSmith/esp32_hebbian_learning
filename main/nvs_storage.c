...
esp_err_t save_gestures_to_nvs(const GestureGraph *graph) {
    ...
    err = nvs_set_blob(nvs_handle, "gesture_graph", graph, sizeof(GestureGraph));
    if (err == ESP_OK) {
        err = nvs_commit(nvs_handle);
    }
    ...
    nvs_close(nvs_handle);
    if (err == ESP_OK) {
        ESP_LOGI(TAG, "Gesture library saved to NVS.");
    }
    return err;
}
...
esp_err_t load_gestures_from_nvs(GestureGraph *graph) {
    ...
    size_t required_size = sizeof(GestureGraph);
    err = nvs_get_blob(nvs_handle, "gesture_graph", graph, &required_size);

    nvs_close(nvs_handle);
    if (err == ESP_OK) {
        ESP_LOGI(TAG, "Gesture library loaded from NVS.");
    }
    return err;
}
...
esp_err_t save_snn_weights_to_nvs(const snn_lsm_t *lsm) {
    ...
    err = nvs_set_blob(nvs_handle, "snn_wout", lsm->w_out, sizeof(lsm->w_out));
    if (err == ESP_OK) {
        err = nvs_commit(nvs_handle);
    }
    ...
    nvs_close(nvs_handle);
    if (err == ESP_OK) {
        ESP_LOGI(TAG, "SNN readout weights saved to NVS.");
    }
    return err;
}
...
esp_err_t load_snn_weights_from_nvs(snn_lsm_t *lsm) {
    ...
    size_t required_size = sizeof(lsm->w_out);
    err = nvs_get_blob(nvs_handle, "snn_wout", lsm->w_out, &required_size);

    nvs_close(nvs_handle);
    if (err == ESP_OK) {
        ESP_LOGI(TAG, "SNN readout weights loaded from NVS.");
    }
    return err;
}
