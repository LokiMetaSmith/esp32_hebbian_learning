...
def main(args):
    ...
    # Step 1: Get the scene image, either from a file or the camera
    if args.scene_path:
        print(f"[INFO] Using provided scene image: {args.scene_path}")
        scene_path = args.scene_path
    else:
        print("[INFO] No scene path provided, using live camera.")
        scene_path = capture_image_from_camera()

    if not scene_path:
        print("\nCould not get a scene image. Exiting.")
        return

    # Step 2: Use the LLM vision model to find the target
    llm = LLMController(api_key=NRP_API_KEY, base_url=BASE_URL)
    target_coords = llm.find_target_with_vision(scene_path, args.target_name)
    if target_coords is None:
        print(f"\n[CV] Could not find '{args.target_name}' in the image. Exiting.")
        return
    ...
    try:
        ...
        command_plan = llm.get_robot_commands(tool_info["tools"], target_coords)
        if not command_plan:
        ...
        print(f"\n[CONTROLLER] Robot is now pointing at {args.target_name}! Mission complete.")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Robotic Pointer Demo with NRP Vision API")
    parser.add_argument("--ip", dest="ip_address", required=True, help="IP address of the ESP32 robot.")
    parser.add_argument("--target-name", dest="target_name", default="Waldo", help="Name of the target to find in the image (e.g., 'Waldo').")
    parser.add_argument("--scene-path", dest="scene_path", default=None, help="Optional path to a static scene image. If not provided, the webcam will be used.")
